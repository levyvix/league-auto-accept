...
from data import load_champions, load_summoner_id, load_summoner_spells
            ...
            with Live(dummy_panel, refresh_per_second=4, console=self.console) as live:
                while self.running:
                    ...
                    if not automation_thread or not automation_thread.is_alive():
                        ...
                        automation_thread = threading.Thread(target=self.auto_accept.run, daemon=True)
                        automation_thread.start()
                        logger.info("Started automation thread")

                    # Show appropriate screen and handle input
                    if self.current_screen == 'main':
                        panel = self.ui.build_main_screen(self.settings, self.auto_accept.shared_state)
                        live.update(panel)
                        self._handle_main_input()

                    elif self.current_screen == 'champion_picker':
                        panel, filtered = self.ui.build_champion_picker("Select Champion", self.settings.recent_champions)
                        live.update(panel)
                        self._handle_picker_input(filtered, 'champ')

                    elif self.current_screen == 'ban_picker':
                        panel, filtered = self.ui.build_champion_picker("Select Ban", self.settings.recent_bans)
                        live.update(panel)
        ...
        if key == 27:  # Esc
            ...
            if msvcrt.kbhit():
                ...
                if next_key == 72:  # Up
                    ...
                    self.ui.current_selection = min(len(filtered) - 1, self.ui.current_selection + 1)
